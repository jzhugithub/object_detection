...
from object_detection.core.layers import gradient_conv2d411
...
class GradientConvolutionalBoxPredictor(BoxPredictor):
  """Gradient Convolutional Box Predictor.
  ...
  def __init__(self,
    ...
    Raises:
      ValueError: if min_depth > max_depth.
    """
    super(GradientConvolutionalBoxPredictor, self).__init__(is_training, num_classes)
    if min_depth > max_depth:
      raise ValueError('min_depth should be less than or equal to max_depth')
    self._conv_hyperparams = conv_hyperparams
    self._min_depth = min_depth
...
class RetinaNetBoxPredictor(BoxPredictor):
  """RetinaNet Box Predictor.

  Optionally add intermediate 3x3 convolutional layer after features and
  predict in parallel branches box_encodings and
  class_predictions_with_background.
